...
class DaWebController extends CController
{
    ...
    public function filterAjaxOnlySilent($filterChain)
    {
        if ( Yii::app()->getRequest()->getIsAjaxRequest() ){
            ...
            throw new DaHttpException(HttpCode::BAD_REQUEST, Yii::t('yii', 'Your request is invalid.'));
        }
    }
    ...
    {
        if ($path == null) {
            $path = $this->getThemeAssetsPath($cssFile);
        }
        if ($path == null) {
    ...
    {
        if ($path == null) {
            $path = $this->getThemeAssetsPath($jsFile);
        }
        if ($path == null){
            $path = $this->getAssetsPath();
        }else if (strpos($path, '/') === false){
    ...
    public function throw404Error($errorMessage = null)
    {
        ...
        throw new CHttpException(HttpCode::NOT_FOUND, $errorMessage);
    }
    ...
    {
        if ($model === null) {
            throw new CHttpException(HttpCode::NOT_FOUND, $notFoundMessage);
        }
        return $model;
    }
}