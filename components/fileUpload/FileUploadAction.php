...
class FileUploadAction extends CAction
{
        ...
        if ($this->beforeUploadValidate()) {
            if ($formModel->validate()) {
                ...
                if (!$this->checkAccess($oldFile)) {
                    throw new CHttpException(HttpCode::FORBIDDEN, 'Вы не можете выполнить данную операцию.');
                }
                $newFile = null;
                ...
                if ($formModel->instanceId) {
                    ...
                    if ($model == null) {
                        throw new CHttpException(HttpCode::NOT_FOUND, "Экземпляр модели id_object = {$formModel->objectId}, с id_instance = {$formModel->instanceId} не найден.");
                    }
                ...
                if (!$uploadedFile->saveAs($path)) {
                    throw new CHttpException(HttpCode::INTERNAL_SERVER_ERROR, "Не удалось сохранить файл '{$path}'.");
                } else {
                    chmod($path, 0777);
                }
                ...
                if (!$newFile->save()) {
                    throw new CHttpException(HttpCode::INTERNAL_SERVER_ERROR, "Не удалось сохранить модель File: \n" . $this->getModelErrors($newFile));
                }
                ...
                if ($formModel->instanceId) {
                    ...
                    if ($objectParameter == null) {
                        throw new CHttpException(HttpCode::NOT_FOUND, "Параметр объекта id_object = {$formModel->objectId}, id_parameter = {$formModel->parameterId} не найден.");
                    }
                ...
                if (isset($formModel->errors['file'])) {
                    throw new DaHttpException(HttpCode::BAD_REQUEST, $this->getModelErrors($formModel)); // без логирования
                } else {
                    throw new CHttpException(HttpCode::BAD_REQUEST, $this->getModelErrors($formModel));
                }
            }
        }
    }
    ...
    protected function handleDelete()
    {
        ...
        if (empty($fileId)) {
            throw new CHttpException(HttpCode::BAD_REQUEST, 'Не указан fileId.');
        }
        if (($file = File::model()->findByPk($fileId)) == null) {
            throw new CHttpException(HttpCode::NOT_FOUND, "Файл с id_file = {$fileId} не найден.");
        }
        ...
        if ($objectParameter->not_null) {
            throw new CHttpException(HttpCode::INTERNAL_SERVER_ERROR, 'Не возможно удалить NOT NULL свойство.');
        }
        if (!$file->delete()) {
            throw new CHttpException(HttpCode::INTERNAL_SERVER_ERROR, "Не удалось удалить файл с id_file = {$fileId }.");
        }
    ...
    protected function createDir($dir)
    {
        ...
        for ($i = 0; $i < $count; $i++) {
            if ( in_array(trim($array[$i]), array('', '.', '..')) ) {
                continue;
            }
            ...
            $permissions = substr(decoct(fileperms($filePath)), 2, 6);
            ...
            if ($permissions != '0777') {
                if (chmod($filePath, 0777) == false){
                    throw new Exception("Права на папку {$filePath} должны быть 0777");
                }
            }
        }
        return $filePath;
