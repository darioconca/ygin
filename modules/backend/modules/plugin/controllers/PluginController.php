...
class PluginController extends DaObjectController
{
    ...
    public function actionView($code)
    {
        ...
        if ($plugin->status != Plugin::STATUS_ENABLE) {
            throw new CHttpException(HttpCode::BAD_REQUEST);
        }
    ...
    public function actionTurnOff($code)
    {
        ...
        if ($plugin->status == Plugin::STATUS_ENABLE) {
            ...
            if ($depends) {
                echo CJSON::encode(array(
                    'error' => "Нет возможности отключить данный плагин, т.к. он используется другими: ".implode(', ',$depends)
                ));
                return;
            }
        ...
        $html = $this->renderPartial('/_buttons', array(
            'plugin' => $plugin
        ), true);
    ...
    public function actionTurnOn($code)
    {
        ...
        $criteria->addInCondition('code', $depends);
        $allPlugin = Plugin::model()->enabled()->findAll($criteria);
        $not = array();
        foreach ($depends as $dependency) {
            foreach ($allPlugin as $current) {
                if ($current->code == $dependency) {
                    continue(2);
                }
            }
            $not[] = $dependency;
