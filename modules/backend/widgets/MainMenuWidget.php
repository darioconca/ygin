    ...
    private function fillItems(array $childs, $level, $availableObjects)
    {
        ...
        foreach ($childs as $child) {
            ...
            if ($level < 2 && $child->countChild > 0) { // объект является папкой
                $items = array_merge($items, $this->fillItems($child->child(array(
                    'with'      => array('views:selectName', 'countChild'),
                    'select'    => 'child.id_object, child.object_type, child.name, child.table_name')), $level + 1, $availableObjects));
            }
        }
        return $items;
    }
