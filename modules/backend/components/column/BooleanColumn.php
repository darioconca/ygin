...
class BooleanColumn extends BaseColumn
{
  ...
  $("#bool_" + data.idInstance + "_" + data.idObjectParameter).removeClass().addClass(newClass);
    ...
    protected function renderDataCellContent($row, $data)
    {
        ...
        $value = $data->$field;
        $htmlOptions = array();
        if ($value == ObjectParameter::PARAMETER_VALUE_FALSE) {
            $htmlOptions['class'] = 'glyphicon glyphicon-remove icon-red';
        }else{
            $htmlOptions['class'] = 'glyphicon glyphicon-ok icon-green';
        }
