...
class BackendModule extends DaWebModuleAbstract implements IApplicationComponent
{ //IBackendExtension
    ...
    public function init()
    {
        ...
        foreach ($this->autoExtensionDir as $dir) {
            ...
            foreach ($files as $file) {
                ...
                $extName = str_replace('config.php', '', str_replace($dir, '', $file));
    ...
    public function setExtensions($extensions)
    {
        if (is_object($extensions)){
            throw new Exception("Type of extensions must be array or string");
        }
        if ($extensions == null){
            return;
        }
        if (is_string($extensions)) {
            ...
            $this->_extensions = CMap::mergeArray($this->_extensions, $extensions);
        }
    }

    public function initExtension($category)
    {
        if (in_array($category, $this->_initExt)){
            return;
        }
        foreach ($this->_extensions as $name => $params) {
            ...
            foreach ($categories as $cat) {
                if (trim($cat) == $category) {
                    ...
                    if ($ext instanceof IBackendExtension){
                        $ext->registerEvent($category, $this);
                    }
                    break;
                }
            }
