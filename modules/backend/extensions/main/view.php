<div class="b-welcome">
    ...
    if ($showWelcome) :
        ...
        <div class="introduction">
            ...
            <p class="about-description">
                Если вам потребуется помощь, посмотрите нашу документацию на странице «<a
                    href="<?php echo Yii::app()->createUrl('instruction'); ?>">Первые шаги с ygin</a>». Если вы хотите
                сразу начать работать, здесь собраны действия, которые большинство пользователей выполняют ежедневно при
                работе с сайтом.
            </p>

            <p style="text-align:right;">Уже всё знаете?
                <button class="btn btn-default btn-xs"
                        onclick="$.cookie('daMainWelcome', 1, {expires:10000, path:'/'}); $(this).parents('.introduction').slideUp()">
    ...
    <?php if (count($devNotices) > 0) : ?>
        <div class="noticeForDev alert alert-info">
            <button type="button"
                    onclick="$.cookie('<?php echo $noticeDevCookieName; ?>', 1, {expires:10000, path:'/'});"
            ...
            <p style="text-align:left;">
                <button class="btn btn-success btn-mini"
                        onclick="$.cookie('<?php echo $noticeDevCookieName; ?>', 1, {expires:10000, path:'/'}); $(this).parents('.noticeForDev').slideUp()">
                    <i class="glyphicon glyphicon-ok"></i> Всё сделано, закрыть сообщение
                </button>
            </p>
        </div>
    <?php endif; ?>
