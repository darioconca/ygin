    ...
    public function processModel(CEvent $event)
    {
        ...
        if (count($columnsForm)) {
            ...
            foreach ($columnsForm as $col) {
                ...
                $column->id_object_view_column  = $idView . '-' . str_replace('_', '-', $objectParam->getFieldName());
                $column->id_object_view         = $idView;
                $column->id_object              = $idObject;
                $column->id_object_parameter    = $objectParam->getIdParameter();
                $column->caption                = $objectParam->getCaption();
                $column->id_data_type           = $objectParam->getType();
                $column->field_name             = $objectParam->getFieldName();
                $column->save();
            }
