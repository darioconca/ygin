...
class DefaultController extends DaObjectController
{
    ...
    private function main()
    {
        ...
        // сортировка грида
        $order = $view->getOrderBy();
        if ($order == null && $object->id_field_order != null) {
            //Если есть порядок сортировки, то добавляем сортировку
            //Загружаем информацию по св-ву сортировки:
            $objectParam = $object->getParameterObjectByIdParameter($object->id_field_order);
            $name = "t." . $objectParam->getFieldName();
  ...
  ' . CHtml::ajax(array(
        ...
        foreach ($columns as $column) {
            ...
            if ( $column->getIdPhpScript() == null ){
                $phpScript = null;
            }else{
                $phpScript = $column->columnClass;
            }
...
', CClientScript::POS_READY);
    ...
    public function actionIndexGroup()
    {
        ...
        if ($object == null){
            $this->_groupInstance = null;
        }else{
            $this->_groupInstance = $object->getModel()->findByIdInstance($grpInst);
        }
