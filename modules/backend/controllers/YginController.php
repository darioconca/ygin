...
class YginController extends DaBackendController
{
    ...
    public function actionSort()
    {
        ...
        try {
            ...
            if ($seqKey == null) {
                throw new Exception('Не найдено поле для сортировки.');
            }
            if ( !is_array($data) ) {
            ...
            foreach ($data as $strId => $value) {
                $idInst = str_replace('ygin_inst_', '', $strId);
    ...
    public function actionDeleteRecord()
    {
        ...
        try {
            ...
            if ($model->delete()) {
                ...
                if ($childCount > 0) {
                    throw new Exception("Информация не удалена, т.к. присутствуют дочерние экземпляры ({$childCount})");
                } else {
