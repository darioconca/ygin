...
if (GET('log') != '') {
    ...
    Выводить строк: <a
        href="<?php echo preg_replace('~&count=\d+~', '', $_SERVER['REQUEST_URI']) ?>&count=1000">1000</a>
    ...
    <table class="table table-bordered b-instance-list" cellspacing="0" cellpadding="0">
        ...
        <tr>
            <th>&nbsp;</th>
            <th><a href="<?php $urlPage->setProperty(DA_URL_OBY, 'file');
                echo $urlPage->getUrl() ?>">Файл</a></th>
            <th><a href="<?php $urlPage->setProperty(DA_URL_OBY, 'mtime');
                echo $urlPage->getUrl() ?>">Изменен</a></th>
            <th><a href="<?php $urlPage->setProperty(DA_URL_OBY, 'size');
                echo $urlPage->getUrl() ?>">Размер</a></th>
        </tr>
        </thead>
        <tbody>
        <?php
        ...
        foreach ($files as $v) {
            ...
            ?>
            <tr class="base">
                <td class="col-ref action-view"><a href="?log=<?php echo $file ?>" title="Изменить"><i>&nbsp;</i></a>
                </td>
