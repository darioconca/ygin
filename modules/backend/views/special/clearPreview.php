...
foreach ($files as $file) {
    ...
    if (!file_exists($file->getFilePath(true))) {
        $idOfNotExistFiles[] = $file->id_file;
        $arrayOfNotExistFiles[] = $path;
    }
}

if (count($arrayOfNoRegisteredFiles) > 0 && $action == 'deleteNoRegister') {
    foreach ($arrayOfNoRegisteredFiles as $k => $file) {
        echo '<br>удаляем ' . $file . '... ';
        echo(unlink(Yii::getPathOfAlias('webroot') . '/' . $file) ? 'успешно!' : 'ошибка!');
