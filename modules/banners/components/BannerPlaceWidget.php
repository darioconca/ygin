...
class BannerPlaceWidget extends DaWidget
{
    ...
    protected function addBannersReplacementCode(Banner $banner, $code)
    {
        ...
        if (self::$interval < self::INTERVAL_DEFAULT) {
            $this->bannersReplacementCode[] = array(
                'id'        => $id,
                'code'      => $code,
                'interval'  => 0
            );
    ...
    public function renderItem(Banner $banner, $index)
    {
        ...
        echo CHtml::openTag($this->itemReplaceWrapper, array(
            'id' => $id
        ));
    ...
    protected function getFileSize(Banner $banner)
    {
        ...
        if ($this->sizeFromBannerPlace) {
            ...
            if ( !file_exists($filePath) ) {
                Yii::log("Ошибка получения размеров файла '{$filePath}': файл не существует ", CLogger::LEVEL_WARNING, 'application.modules.banners');
                return false;
            }
            if ( !($size = getimagesize($filePath)) ) {
                Yii::log("Ошибка получения размеров файла '{$filePath}': ".print_r($size,true), CLogger::LEVEL_WARNING, 'application.modules.banners');
                return false;
            }
        }
    ...
    protected function getImageBanner(Banner $banner, $width = null, $height = null)
    {
        ...
        if (
            ...
            $code = CHtml::link($code,$clickUrl,array(
                'title' => $banner->alt
            ));
        }
        return $code;
