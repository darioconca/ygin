...
<table class="b-emarket_list_table table table-striped">
    <?php foreach ($products as $product) {
        ...
        ?>
        <tr>
            <td class="preview_picture">
                <a href="<?php echo $url; ?>">
                    <img src="<?= $previewUrl ?>" alt="<?= CHtml::encode($product->name) ?>" title="<?= CHtml::encode($product->name) ?>">
                </a>
            </td>
            <td class="elem">
            ...
            <td class="remain_status"><i
                    class="glyphicon glyphicon-signal <?php echo $product->getRemainStatus()->icon; ?>"
                    title="количество товара: <?php echo $product->getRemainStatus()->name; ?>"></i></td>
            <td class="buttons">
                <button data-name="<?php echo CHtml::encode($product->name); ?>"
                        data-id="<?php echo $product->id_product; ?>" data-price="<?php echo $price; ?>"
                        class="btn btn-success buy"><i class="glyphicon glyphicon-shopping-cart"></i> В корзину
                </button>
            </td>
        </tr>
    <?php } ?>
