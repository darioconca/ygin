  ...
  $(".b-cart").Cart(' .
    CJavaScript::encode(array(
      ...
      DaCart.removeProduct(data.item.attr("data-id"));
        ...
        'itemTemplate' => "<li data-price-result=\"\" data-qty=\"\" data-price=\"\" data-id=\"\" class=\"item\">
...
<div class="b-cart well">
    ...
    <ul class="tovarList">
        <?php foreach ($products as $product){ ?>
            <li data-qty="<?= $product->countInCart ?>" data-id="<?= $product->id_product ?>"
                data-price="<?= $product->getPriceWithMarkup() ?>" class="item">
                ...
                <div class="price <?= $noPriceClass ?>">
                    <span class="val">0</span> <img src="<?= $currencyImage ?>" alt="руб." title="руб.">
                </div>
            </li>
        <?php } ?>
    </ul>
    <div class="itogo alert <?= $noPriceClass ?>">
        <table cellpadding="0" cellspacing="0" style="width:100%">
            <tr>
                <th style="width:60%">Итог</th>
    ...
    <div class="btns <?= $noPriceClass ?>">
        <a class="btn btn-large btn-success offer" href="<?= Yii::app()->urlManager->createUrl(ShopModule::ROUTE_OFFER) ?>">
            <i class="icon-shopping-cart icon-white"></i> Оформить заказ
        </a>
        <button class="btn clear btn-link">
