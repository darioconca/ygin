...
<?php if (file_exists($this->getLocalConfigFile())){ ?>
    <div class="alert alert-danger">
        Файл с конфигурацией <?= $this->getLocalConfigFile() ?> уже существует.
...
<?php if (empty($this->dumpFiles)){ ?>
    <div class="alert alert-danger">
        Файл <?= $this->getDumpFile() ?> с дампом базы отсутствует. Установка невозможна.
    </div>
    <?php return; ?>
<?php } ?>
    ...
    <div class="tab-content">
        <div class="tab-pane" id="step1">
            <table class="table table-bordered">
                ...
                <?php foreach ($requirements as $requirement){ ?>
                    <tr class="<?= $requirement['pass'] ? 'success' : 'error' ?>">
        ...
        <div class="tab-pane" id="step2">
            ...
            <div class="control-group">
                <?= $form->labelEx($dbSettings, 'host', array(
                    'class' => 'control-label'
                )); ?>
                <div class="controls">
                    <?= $form->textField($dbSettings, 'host', array(
                        'placeholder' => 'localhost'
                    )); ?>
                    <?= $form->error($dbSettings, 'host'); ?>
                </div>
            </div>
            <div class="control-group">
                <?= $form->labelEx($dbSettings, 'port', array(
                    'class' => 'control-label'
                )); ?>
                <div class="controls">
                    <?= $form->textField($dbSettings, 'port'); ?>
                    <?= $form->error($dbSettings, 'port'); ?>
                </div>
            </div>
            <div class="control-group">
                <?= $form->labelEx($dbSettings, 'dbname', array(
                    'class' => 'control-label'
                )); ?>
                <div class="controls">
                    <?= $form->textField($dbSettings, 'dbname'); ?>
                    <?= $form->error($dbSettings, 'dbname'); ?>
                </div>
            </div>
            <div class="control-group">
                <?= $form->labelEx($dbSettings, 'user', array(
                    'class' => 'control-label'
                )); ?>
                <div class="controls">
                    <?= $form->textField($dbSettings, 'user'); ?>
                    <?= $form->error($dbSettings, 'user'); ?>
                </div>
            </div>
            <div class="control-group">
                <?= $form->labelEx($dbSettings, 'password', array(
                    'class' => 'control-label'
                )); ?>
                <div class="controls">
                    <?= $form->passwordField($dbSettings, 'password'); ?>
                    <?= $form->error($dbSettings, 'password'); ?>
                </div>
            </div>
            <div class="control-group">
                <div class="controls">
                    <?= CHtml::label($form->checkBox($dbSettings, 'createDb') . $dbSettings->getAttributeLabel('createDb'),
                        CHtml::activeId($dbSettings, 'createDb'),array(
                        'class' => 'checkbox'
                    )); ?>
                    <?= $form->error($dbSettings, 'password'); ?>
                </div>
        ...
        <div class="tab-pane" id="step3">
            ...
            <div class="control-group">
                <?= $form->labelEx($userSettings, 'fullName', array(
                    'class' => 'control-label'
                )); ?>
                <div class="controls">
                    <?= $form->textField($userSettings, 'fullName'); ?>
                    <?= $form->error($userSettings, 'fullName'); ?>
                </div>
            </div>
            <div class="control-group">
                <?= $form->labelEx($userSettings, 'name', array(
                    'class' => 'control-label'
                )); ?>
                <div class="controls">
                    <?= $form->textField($userSettings, 'name'); ?>
                    <?= $form->error($userSettings, 'name'); ?>
                </div>
            </div>
            <div class="control-group">
                <?= $form->labelEx($userSettings, 'email', array(
                    'class' => 'control-label'
                )); ?>
                <div class="controls">
                    <?= $form->textField($userSettings, 'email'); ?>
                    <?= $form->error($userSettings, 'email'); ?>
                </div>
            </div>
            <div class="control-group">
                <?= $form->labelEx($userSettings, 'password', array(
                    'class' => 'control-label'
                )); ?>
                <div class="controls">
                    <?= $form->passwordField($userSettings, 'password'); ?>
                    <?= $form->error($userSettings, 'password'); ?>
                </div>
            </div>
            <div class="control-group">
                <?= $form->labelEx($userSettings, 'passwordRepeat', array(
                    'class' => 'control-label'
                )); ?>
                <div class="controls">
                    <?= $form->passwordField($userSettings, 'passwordRepeat'); ?>
                    <?= $form->error($userSettings, 'passwordRepeat'); ?>
                </div>
            </div>
            <?php $this->endWidget(); ?>
        </div>
        <div class="tab-pane tab-finish" id="tabFinish">
            <div id="install-detail">
                ...
                <button type="submit" class="btn btn-success btn-large pull-right btn-finish">Завершить установку
                </button>
            </div>
...
<script type="text/javascript">
    ...
    $(document).ajaxStart(function () {
        ...
        $('#loader-overlay').offset($wizard.offset()).width($wizard.width()).height($wizard.height());
        $('<img id="loader" src="<?= CHtml::asset($this->module->getBasePath().'/assets/ajax-loader.gif') ?>" >').appendTo('body');
        $('#loader').offset({
            top: $wizard.offset().top + ($wizard.height() / 2),
            left: $wizard.offset().left + ($wizard.width() / 2)
        });
    });
