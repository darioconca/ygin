...
class DefaultController extends CController
{
    ...
    public function filtercheckDir($filterChain)
    {
        ...
        if (!TransferData::isResourceValid($_SERVER['HTTP_HOST'] . $this->createUrl('step2'))) {
            ...
            throw new CHttpException(HttpCode::GATEWAY_TIMEOUT, 'Система не смогла выполнить внутренние запросы.<br> Такое может произойти если система установлена в подпапку сайта (domain.ru/install_files/), либо при некорректной работе .htaccess файла');
        }
        $filterChain->run();
    }
    ...
    public function actionIndex()
    {
        $requirements = array(
            ...
            array(
                'require' => 'mbstring',
                'current' => extension_loaded('mbstring') ? Yii::t('backend', 'Yes') : Yii::t('backend', 'No'),
                'pass' => extension_loaded('mbstring'),
            )
        ,
            array(
                'require' => 'pdo_mysql',
                'current' => extension_loaded('pdo_mysql') ? Yii::t('backend', 'Yes') : Yii::t('backend', 'No'),
    ...
    public function filterCheck($filterChain)
    {
        if ($error = $this->validate()) {
            throw new CHttpException(HttpCode::INTERNAL_SERVER_ERROR, $error);
        }
        $filterChain->run();
    }
    ...
    public function actionImportDump()
    {
        if (!($dumpFile = (int)Yii::app()->request->getPost('dumpFile'))) {
            throw new CHttpException(HttpCode::BAD_REQUEST, 'Не указан файл дампа.');
        }
        $dumps = $this->getDumpFiles();
        if ($dumpFile < 1 || $dumpFile > count($dumps)) {
            throw new CHttpException(HttpCode::BAD_REQUEST, 'Неверный файл дампа.');
        }
        $dbSettings = $this->getDbSettingsModel();
    ...
    public function actionCreateUser()
    {
        ...
        if (!$userModel->save()) {
            ...
            $trans->rollback();
            throw new CHttpException(HttpCode::INTERNAL_SERVER_ERROR, "Не удалось добавить пользователя:\n" . print_r($errors, true));
        } else {
    ...
    public function actionError()
    {
        ...
        if ($error = Yii::app()->errorHandler->error) {
            if ($error['code'] == HttpCode::NOT_FOUND) { // Устанавливаем свой макет для отображения 404 ошибки
