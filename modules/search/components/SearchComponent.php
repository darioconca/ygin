...
class SearchComponent extends CApplicationComponent
{
    ...
    public function addObjectInSearchList($idObject)
    {
        if (!in_array($idObject, $this->objectSearchList)) {
            ...
            end($this->objectSearchList); // move the internal pointer to the end of the array
            return key($this->objectSearchList);
        }else{
            return null;
        }
    }
    ...
    public function startSearch($query)
    {
        ...
        if (
            is_null($this->lenPreviewText) ||
            !is_numeric($this->lenPreviewText) ||
            ($this->lenPreviewText < 1)
        ) {
        ...
        $results = Search::model()->findAll($criteria);
        foreach ($results as $result) {
            $result->value = HText::highlightText($result->value, $this->queryArray, $this->highlightTemplate, $this->lenPreviewText);
        }
        return $results;
    }
    ...
    public static function replaceIndex(DaActiveRecord $instance, $idLang = 1)
    {
        ...
        return self::replaceData($instance->getIdObject(), $instance->getIdInstance(), $idLang, $data);
    }
    ...
    {
        $preparedValue = SearchComponent::processValue($data);

        $prs = Yii::app()->db->createCommand()
            ->select('id_object')
            ->from('da_search_data')
            ->where('id_object = :obj AND id_instance = :inst AND id_lang = :lang', array(
                ':obj'  => $idObject,
                ':inst' => $idInstance,
                ':lang' => $idLang
            ))
    ...
    /**
     * @param int $idLang
     * @param array $arrayWithBadObject
     */
    public static function recreateIndex($idLang = 1, $arrayWithBadObject = array())
    {
        // Удаляем старые данные
        Yii::app()->db
            ->createCommand('DELETE FROM da_search_data WHERE id_lang=:id_lang')
            ->execute(array(
                ':id_lang' => $idLang
            ));
