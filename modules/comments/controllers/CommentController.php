...
class CommentController extends Controller
{
    ...
    public function actionUpdateComment($idObj = null, $idInst = null)
    {
        ...
        Yii::app()->db
            ->createCommand($sql)
            ->execute(array($idObject, $idInstance, Yii::app()->user->id, time(), time()));
    }
    ...
    public function loadModel($id)
    {
        ...
        if ($model === null){
            throw new CHttpException(HttpCode::NOT_FOUND, 'The requested page does not exist.');
        }
        return $model;
    }
    ...
    public function actionSetStatusApproveComment()
    {
        ...
        if ($comment != null && Yii::app()->user->checkAccess('approveComment', array(
            'comment' => $comment,
        ))) {
            $comment->setApproved();
            return $this->getSuccessResultData($comment);
