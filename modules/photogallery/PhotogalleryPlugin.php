...
class PhotogalleryPlugin extends PluginAbstract
{
    ...
    public function getConfigByParamsValue(array $paramsValue, $data)
    {
        ...
        $config = array();
        if (!$useGallery) {
            $config['urlRules'] = array(
                'photogallery' => null,
                'photogallery/<idGallery:\d+>' => null
            );
        }
        return array(
            'modules' => array('ygin.photogallery' => $config)
        );
    }
    ...
    public function activate(Plugin $plugin)
    {
        ...
        // галерея
        if (!isset($data['useGallery']) || $data['useGallery'] === false) {
            return;
        }
