...
class RandomPhotoWidget extends DaWidget implements IParametersConfig
{
    ...
    public function run()
    {
        ...
        if (Photogallery::model()->count() > 1) {
            ...
            $galleryLink = Yii::app()->createUrl(PhotogalleryModule::ROUTE_GALLERY_VIEW, array(
                'idGallery' => $model->id_photogallery_instance
            ));
        }
        if ($preview !== null) {
            $photoLink = Yii::app()->createUrl(PhotogalleryModule::ROUTE_GALLERY_VIEW, array(
                'idGallery' => $model->id_photogallery_instance
            ));
        }
