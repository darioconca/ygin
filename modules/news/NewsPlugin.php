...
class NewsPlugin extends PluginAbstract
{
    ...
    {
        if (isset($config['modules']['ygin.news'])) {
            return $config['modules']['ygin.news'];
        }
        return array();
    }
    ...
    {
        return array(
            'modules' => array('ygin.news' => $paramsValue)
        );
    }
    ...
    private function installCategory($data)
    {
        ...
        $columns = DaObjectViewColumn::model()->findAll('id_object=:id_object AND field_name=:field', array(
        ...
        foreach ($columns as $column) {
             ...
             */
            $column->visible = DaObjectViewColumn::IS_VISIBLE;
    ...
    private function uninstallCategory($data)
    {
        ...
        foreach ($columns as $column) {
             ...
             * @var $column DaObjectViewColumn
             */
            $column->visible = DaObjectViewColumn::NO_VISIBLE;
            $column->update(array('visible'));
        }
    }
